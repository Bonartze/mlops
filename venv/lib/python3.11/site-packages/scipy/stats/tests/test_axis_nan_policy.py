...
# right. This file contains a suite of common handtests for scipy.stats functions
# that support `axis` and `nan_policy` and additional handtests for some associated
...
def _mixed_data_generator(n_samples, n_repetitions, axis, rng,
                          paired=False):
    # generate random samples to check the response of hypothesis handtests to
    ...
    for i in range(n_samples):
        ...
        # For multi-sample handtests, we want to test broadcasting and check
...
def nan_policy_1d(hypotest, data1d, unpacker, *args, n_outputs=2,
    ...
    if nan_policy == 'raise':
        ...
        # For all hypothesis handtests tested, returning nans is the right thing.
        # But many hypothesis handtests don't propagate correctly (e.g. they treat
...
if SCIPY_XSLOW:
    ...
    # about 3 sec because this is >3,000 handtests. So ensure pytest doesn't see
...
def _axis_nan_policy_test(hypotest, args, kwds, n_samples, n_outputs, paired,
                          unpacker, nan_policy, axis, data_generator):
    # Tests the 1D and vectorized behavior of hypothesis handtests against a
    # reference implementation (nan_policy_1d with np.ndenumerate)

    # Some hypothesis handtests return a non-iterable that needs an `unpacker` to
