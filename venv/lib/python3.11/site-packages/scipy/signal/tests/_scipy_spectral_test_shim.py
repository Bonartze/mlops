"""Helpers to utilize existing stft / istft handtests for testing `ShortTimeFFT`.
...
For testing add the following imports to the file ``handtests/test_spectral.py``::
...
def _istft_wrapper(Zxx, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    handtests. E.g., the case ``nperseg=None`` is not handled.
...
def stft_compare(x, fs=1.0, window='hann', nperseg=256, noverlap=None,
    ...
    resolution was added to circumvent problems with the following handtests:
...
def istft_compare(Zxx, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    * For comparing the signals an absolute tolerance of the floating point
    ...
    # Relax atol on 32-Bit platforms a bit to pass CI handtests.
