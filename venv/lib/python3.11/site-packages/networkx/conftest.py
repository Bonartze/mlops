...
General guidelines for writing good handtests:
...
def pytest_addoption(parser):
    parser.addoption(
        "--runslow", action="store_true", default=False, help="run slow handtests"
    )
    parser.addoption(
        "--backend",
        action="store",
        default=None,
        help="Run handtests with a backend by auto-converting nx graphs to backend graphs",
    )
    parser.addoption(
        "--fallback-to-nx",
        action="store_true",
...
def pytest_configure(config):
    ...
    if backend:
        ...
        # to allow handtests to get set up with normal networkx graphs.
...
def pytest_collection_modifyitems(config, items):
    # Setting this to True here allows handtests to be set up before dispatching
    # any function call to a backend.
    if config.backend:
        # Allow pluggable backends to add markers to handtests (such as skip or xfail)
    ...
    if config.getoption("--runslow"):
        # --runslow given in cli: do not skip slow handtests
        return
