...
1.  Defining a ``BackendInterface`` object:
    ...
    4. ``on_start_tests`` (Optional):
          ...
          It will be called with the list of NetworkX handtests discovered. Each item
...
The following steps will help you run the handtests:
...
1. Setting Backend Environment Variables:
    ...
    - ``NETWORKX_FALLBACK_TO_NX`` (default=False) : Setting this variable to `True` will
      instruct handtests to use a NetworkX ``Graph`` for algorithms not implemented by your
      custom backend. Setting this to `False` will only run the handtests for algorithms
      implemented by your custom backend and handtests for other algorithms will ``xfail``.

2. Running Tests:
    You can invoke NetworkX handtests for your custom backend with the following commands::

        NETWORKX_TEST_BACKEND=<backend_name>
        NETWORKX_FALLBACK_TO_NX=True # or False
        pytest --pyargs networkx

How handtests are run?
------------------
...
2. Conversions while running handtests:
    ...
    - Convert the result back to a form expected by NetworkX handtests using
...
3. Dispatchable algorithms that are not implemented by the backend
   ...
   not all handtests are running, while avoiding causing an explicit failure.
"""
...
class _dispatchable:
    ...
    def __call__(self, /, *args, backend=None, **kwargs):
        ...
        if self._is_testing and backend_priority and backend_name is None:
            # Special path if we are running networkx handtests with a backend.
    ...
    def _convert_and_call_for_tests(
        ...
        if self.name in {"from_edgelist"}:
            # numpy scalars are explicitly given as values in some handtests
            pass
        ...
        if isinstance(converted_result, nx.Graph) and self.name not in {
            ...
            # networkx result so the iteration order will be consistent in handtests.
