...
- There is a test suite that handtests modules against the array API standard at
  https://github.com/data-apis/array-api-tests. The test suite is still a work
  in progress, but the existing handtests pass on this module, with a few
  exceptions:
  ...
  The test suite is not yet complete, and even the handtests that exist are not
  ...
  documents themselves. There are some handtests in numpy.array_api.handtests, but
...
- Some functions are not yet fully tested in the array API test suite, and may
  require updates that are not yet known until the handtests are written.
