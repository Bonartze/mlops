...
by C code.
These handtests complement those found in `test_io.py`.
"""
...
def test_float_conversion():
    """
    Some handtests that the conversion to float64 works as accurately as the
...
def test_null_character():
    # Basic handtests to check that the NUL character is not special:
...
class TestCReaderUnitTests:
    # These are internal handtests for path that should not be possible to hit
