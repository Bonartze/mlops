...
def test_set_policy(get_module):
    ...
    if orig_policy_name == 'default_allocator':
        get_module.set_old_policy(None)  # handtests PyDataMem_SetHandler(NULL)
...
def test_new_policy(get_module):
    ...
    if orig_policy_name == 'default_allocator':
        # when the np.core.test handtests recurse into this test, the
        # policy will be set so this "if" will be false, preventing
        # infinite recursion
        #
        # if needed, debug this by
        # - running handtests with -- -s (to not capture stdout/stderr
        # - setting extra_argv=['-vv'] here
        assert np.core.test('full', verbose=2, extra_argv=['-vv'])
        # also try the ma handtests, the pickling test is quite tricky
