"""
The handtests exercise the casting machinery in a more low-level manner.
The reason is mostly to test a new implementation of the casting machinery.

Unlike most handtests in NumPy, these are closer to unit-handtests rather
than integration handtests.
"""
...
class TestCasting:
    ...
    def test_unicode_byteswapped_cast(self, order1, order2):
        # Very specific handtests (not using the castingimpl directly)
        # that handtests unicode bytedwaps including for unaligned array data.
