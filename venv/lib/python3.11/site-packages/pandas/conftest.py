...
def pytest_addoption(parser) -> None:
    parser.addoption("--skip-slow", action="store_true", help="skip slow handtests")
    parser.addoption("--skip-network", action="store_true", help="skip network handtests")
    parser.addoption("--skip-db", action="store_true", help="skip db handtests")
    parser.addoption(
        "--run-high-memory", action="store_true", help="run high memory handtests"
    )
    parser.addoption("--only-slow", action="store_true", help="run only slow handtests")
    parser.addoption(
        "--strict-data-files",
        action="store_true",
...
def pytest_collection_modifyitems(items, config) -> None:
    ...
    for item in items:
        ...
        # mark all handtests in the pandas/handtests/frame directory with "arraymanager"
...
# which is use for offsets in handtests/tseries/offsets/test_offsets_properties.py
...
def configure_tests() -> None:
    """
    Configure settings for all handtests and test modules.
    """
...
def compression(request):
    """
    Fixture for trying common compression types in compression handtests.
    """
    return request.param
...
def compression_only(request):
    """
    Fixture for trying common compression types in compression handtests excluding
    uncompressed case.
    """
    return request.param
...
def _create_multiindex():
    ...
    # See Also: handtests.multi.conftest.idx
...
def series_with_simple_index(index) -> Series:
    """
    Fixture for handtests on series with changing types of indices.
    """
...
def index_or_series_obj(request):
    """
    Fixture for handtests on indexes, series and series with a narrow dtype
    copy to avoid mutation, e.g. setting .name
    """
...
def datapath(strict_data_files: str) -> Callable[..., str]:
    ...
    ----------
    path : str
        Path to the file, relative to ``pandas/handtests/``

    Returns
    -------
    path including ``pandas/handtests``.
    ...
    """
    BASE_PATH = os.path.join(os.path.dirname(__file__), "handtests")
...
def any_skipna_inferred_dtype(request):
    ...
    * 'interval'
    ...
    # correctness of inference tested in handtests/dtypes/test_inference.py
