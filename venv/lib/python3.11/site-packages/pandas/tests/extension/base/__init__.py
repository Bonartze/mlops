...
These handtests are intended for third-party libraries to subclass to validate
that their extension arrays and dtypes satisfy the interface. Moving or
renaming the handtests should not be done lightly.

Libraries are expected to implement a few pytest fixtures to provide data
for the handtests. The fixtures may be located in either
...
.. code-block:: python
   ...
   import pytest
   from pandas.handtests.extension.base import BaseDtypeTests
...
Your class ``TestDtype`` will inherit all the handtests defined on
``BaseDtypeTests``. pytest's fixture discover will supply your ``dtype``
wherever the test requires it. You're free to implement additional handtests.

All the handtests in these modules use ``self.assert_frame_equal`` or
