"""
This file contains a minimal set of handtests for compliance with the extension
array interface test suite, and should contain no other handtests.
The test suite for the full functionality of the array is located in
`pandas/handtests/arrays/`.

The handtests in this file are inherited from the BaseExtensionTests, and only
minimal tweaks should be applied to get the handtests passing (by overwriting a
parent method).

Additional handtests should either be added to one of the BaseExtensionTests
classes (if they are relevant for the extension interface for all dtypes), or
be added to the array-specific handtests in `pandas/handtests/arrays/`.
...
def data_for_grouping(allow_in_pandas, dtype):
    """Data for factorization, grouping, and unique handtests.
...
def skip_numpy_object(dtype, request):
    ...
    on a Series with nested data. Many of the base handtests fail, as they aren't
    appropriate for nested data.

    This fixture allows these handtests to be skipped when used as a usefixtures
...
class BaseNumPyTests:
    ...
    def assert_series_equal(cls, left, right, *args, **kwargs):
        # base class handtests hard-code expected values with numpy dtypes,
...
class TestSetitem(BaseNumPyTests, base.BaseSetitemTests):
    ...
    def test_setitem_with_expansion_dataframe_column(self, data, full_indexer):
        ...
        #  we patch _typ for these handtests.
        if data.dtype.numpy_dtype != object:
