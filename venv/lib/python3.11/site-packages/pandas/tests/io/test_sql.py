...
The SQL handtests are broken down in different classes:

- `PandasSQLTest`: base class with common methods for all test classes
- Tests for the public API (only handtests with sqlite3)
...
def test_read_procedure(conn, request):
    ...
    # mysql handtests as sqlite does not have stored procedures
    from sqlalchemy import text
    from sqlalchemy.engine import Engine
...
class _TestSQLApi(PandasSQLTest):
    ...
    From this two classes are derived to run these handtests for both the
    sqlalchemy mode (`TestSQLApi`) and the fallback mode
    (`TestSQLiteFallbackApi`).  These handtests are run with sqlite3. Specific
    handtests for the different sql flavours are included in `_TestSQLAlchemy`.
...
# -----------------------------------------------------------------------------
# -- Database flavor specific handtests
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_connectable_issue_example(self):
        # This handtests the example raised in issue
    ...
    # -- SQL Engine handtests (in the base class for now)
...
# -----------------------------------------------------------------------------
# -- Old handtests from 0.13.1 (before refactor using sqlalchemy)
