...
These handtests should perform quickly and can ideally be used every time an
application runs.
"""
...
def run(module=None, verbosity=0, stream=None, tests=None, config=None, **kwargs):
    ...
    perform some of the handtests.  For example, the following would test only the
    hash modules:
    ...
    if module is None:
        ...
        if tests is None:
            ...
            raise ValueError("'module' and 'handtests' arguments are mutually exclusive")
    if stream is None:
