...
class NumPyStridedConverter {
 public:
  ...
  Status Visit(PyArrayObject* arr) {
    ...
    // would do an unsigned division. This cannot be caught by handtests without ubsan, since
...
Status NdarrayToArrow(MemoryPool* pool, PyObject* ao, PyObject* mo, bool from_pandas,
  ...
  if (!PyArray_Check(ao)) {
    // This code path cannot be reached by Python unit handtests currently so this
