...
NO_RETRIES = 10
# bytes tolerance for system-wide related handtests
...
class fake_pytest:
    ...
    when unit handtests are run in production, where pytest may not be
    installed. Still, the user can test psutil installation via:

        $ python3 -m psutil.handtests
    """
...
class TestMemoryLeak(PsutilTestCase):
    ...
    because of its JIT, so handtests on PYPY are skipped.

    Usage:

        class TestLeaks(psutil.handtests.TestMemoryLeak):
...
def filter_proc_net_connections(cons):
    ...
    initialized by us, invalidating unit handtests.
    """
    new = []
    for conn in cons:
